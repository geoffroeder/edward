...
class test_integer_init(tf.test.TestCase):
  ...
  def test_integer(self):
    ...
    W_1 = Normal(loc=tf.zeros([n_hidden, K]), scale=tf.ones([n_hidden, K]))
    ...
    qW_0 = Normal(loc=tf.Variable(tf.random_normal([D, n_hidden])),
                  scale=softplus(tf.Variable(tf.random_normal([D, n_hidden]))))
    qW_1 = Normal(loc=tf.Variable(tf.random_normal([n_hidden, K])),
                  scale=softplus(tf.Variable(tf.random_normal([n_hidden, K]))))
    qb_0 = Normal(loc=tf.Variable(tf.random_normal([n_hidden])),
                  scale=softplus(tf.Variable(tf.random_normal([n_hidden]))))
    qb_1 = Normal(loc=tf.Variable(tf.random_normal([K])),
                  scale=softplus(tf.Variable(tf.random_normal([K]))))
